...
$icheck=mysql_query("SELECT * FROM smp_ad");
if($icheck){
if($action== "logout"){
session_destroy();
...
?>										
<HTML><HEAD><TITLE>Smart PHP Poll - Administration Panel</TITLE>
<LINK 
href="style.css" 
...
<TD width=75% style="padding-left:5px;BORDER-LEFT:#b6b6b6 1px solid;BACKGROUND-COLOR: #e3e3e3;"><font face=arial size=5 color=#330033><B><I>Smart PHP Poll</I></B></font>
...
if($ser=="true"){
$bro=mysql_query("UPDATE smp_cust SET pw='$pw',boc='$boc',bbc='$bbc',hlc='$hlc',hls='$hls',ttc='$ttc',bts='$bts',btc='$btc',buc='$buc'");
if($bro){
...
$gett=mysql_query("SELECT * FROM smp_cust");
...
echo "<TR height=\"40\"><TD width=20% style=\"padding:4px;\">Title of Poll:</TD><TD width=50%><input type=text name=title size=50></TD></TR>";
...
echo "<TR><TD width=100><input type=hidden name=question value=0><input type=hidden name=no value=\"$no\"><input type=hidden name=title value=\"$title\"></TD><TD width=500><input type=submit value=' Finish '></TD></TR>";
...
$mfr=mysql_query("INSERT INTO smp_quiz SET title='$title', tm='$tm'");
for($i=1;$i<=$no;$i++){
$k=$i;
...
$answer[$i]=str_replace("<","&lt;",$answer[$i]);
$answer[$i]=str_replace(">","&gt;",$answer[$i]);
$mfr=mysql_query("INSERT INTO smp_answer SET answer='$answer[$i]',qid='$i', tm='$tm'");
}
if($mfr){
...
$mfr=mysql_query("DELETE FROM smp_result WHERE tm='$id'");
if($mfr){
...
$mit=mysql_query("SELECT * FROM smp_quiz WHERE tm='$id'");
$qz=mysql_fetch_array($mit);
$tot=mysql_query("SELECT * FROM smp_result WHERE tm='$id'");
...
$mil=mysql_query("SELECT * FROM smp_answer WHERE tm='$id'");
while($cow=@mysql_fetch_array($mil)){
$nm=mysql_query("SELECT * FROM smp_result WHERE point='$cow[mini]' AND tm='$id'");
...
$title=str_replace(">","&gt;",$title);
$title=stripslashes($title);
$er=mysql_query("UPDATE smp_quiz SET title='$title' WHERE tm='$id'");
$mit1=mysql_query("SELECT * FROM smp_answer WHERE tm='$id'");
...
$answer[$ig]=str_replace("<","&lt;",$answer[$ig]);
$answer[$ig]=str_replace(">","&gt;",$answer[$ig]);
$mpb=mysql_query("UPDATE smp_answer SET answer='$answer[$ig]' WHERE tm='$id' AND op='$ig'");
}
...
$mr=mysql_query("SELECT * FROM smp_quiz WHERE tm='$id'");
$in=@mysql_fetch_array($mr);
echo "<TR height=\"10\"><TD colspan=2></TD></TR>";
echo "<TR height=\"35\"><TD width=100 style=\"padding:4px;\">Title of Poll:</TD><TD width=500><input type=text name=title size=50 value=\"$in[title]\"></TD></TR>";
echo "<TR><TD colspan=2><BR></TD></TR>";
$ques=mysql_query("SELECT * FROM smp_answer WHERE tm='$id'");
$i=0;
...
$del=mysql_query("DELETE FROM smp_quiz WHERE tm='$id'");
$del=mysql_query("DELETE FROM smp_answer WHERE tm='$id'");
$del=mysql_query("DELETE FROM smp_result WHERE tm='$id'");
if($del){
...
$exis=mysql_query("SELECT * FROM smp_ad");
$einf=mysql_fetch_array($exis);
if($mode=="verify"){
if($admin_pass==$cpass && strlen($admin_pass)>=1){
$updf=mysql_query("UPDATE smp_ad SET admin_id='$admin_id', admin_pass='$admin_pass'");
...
<TR>
<TD width=20></TD>
<TD>Copyright © <A HREF="http://www.scriptsez.net">Smart PHP Poll</A>. All Rights Reserved. </TD>
...
echo "<table width=40% align=center cellspacing=0><TR bgcolor=\"#E6e6e6\" height=\"26\"><TD align=center width=100% colspan=2><b>ERROR</b></TD></TR><TR bgcolor=\"#F6f6f6\"><TD align=center style=\"BORDER-LEFT:#E6e6e6 1px solid;BORDER-BOTTOM:#E6e6e6 1px solid;\"><img src=\"images/warning.png\"></TD><TD align=center width=80% style=\"BORDER-RIGHT:#E6e6e6 1px solid;BORDER-BOTTOM:#E6e6e6 1px solid;\">Please run installation file first, and follow the instructions given in installation file</TD></TR></table><BR>";
}	
?>